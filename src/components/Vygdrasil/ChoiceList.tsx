          ...
          return (
            <li key={item.id}>
              <button
                ...
                className={`text-left text-lg font-bold transition duration-200 ease-in-out ${
                  isItemDisabled
                    ? "text-gray-500 cursor-not-allowed line-through opacity-50"
                    : "text-bronze hover:text-gray-300"
                }`}
                title={disabled ? "부활 처리 중입니다..." : item.isDisabledByVote ? "투표에서 선택되지 않은 선택지입니다" : isVoting ? "DAO 투표 대상 선택지입니다" : ""}
              >
                &gt; {item.choice}
                {isVoting && <span className="ml-2 text-xs text-yellow-600">🗳️</span>}
                {item.isDisabledByVote && (
                  <span className="ml-2 text-xs text-gray-600">(투표 미선택)</span>
                )}
              </button>
            </li>
          );
